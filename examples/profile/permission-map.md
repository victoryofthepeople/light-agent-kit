...
- Calendar later, if useful for planning.
...
- Project notes.
- Daily notes.
- Power activation cards.

### Safe Quiet Updates

- Idea inbox.
- Lessons after explicit correction.
- Home summary from approved workspace files.
...
| --- | --- | --- |
| Obsidian | local files only | no sync yet |
| Calendar | no access | maybe read-only later |
| Gmail | no access | drafts only for now |
