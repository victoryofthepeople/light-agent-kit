...
Light Agent Kit helps an AI agent work with a person, business, client, or project through plain Markdown files the user can read.
...
The agent brings the intelligence. The kit gives it the missing context: who it is helping, what matters, what good work looks like, which workflows to repeat, and what needs a clear yes before acting.

## The Short Version

The kit helps the agent answer:
...
The agent can build a clear picture of:
...
- What decisions or tradeoffs are active.
...
The maps should include real agent powers, not just folders:
...
- A client follow-up loop.
- A research sprint.
- A build session.
- An "only when I ask" mode.
...
When the user corrects the assistant, the kit gives the agent a place to save the lesson.
...
### Capture, Organize, And Reuse Information

The agent can catch messy thoughts, then route them into:
...
The kit supports two levels:

- Simple notes.
- Source-backed research with sources, briefs, concept pages, and a research log.

The point is to help the agent turn messy inputs into reusable understanding and useful output.
...
Depending on the runtime and access, the agent can help produce:
...
Skills are standing operating procedures.

They let the agent repeat useful workflows instead of improvising every time.
...
- Notes app workflows.
- Phone access.
- External vault or project brain.
...
- `POWER-CATALOG.md`
- `POWER-RECIPES.md`
- `guides/runtime-decision-guide.md`
...
The kit gives the agent the context, rules, and structure to use those powers more clearly when the user chooses to add them.
...
Light Agent Kit gives the agent a readable workspace for this:
